...
// 修改后的脚本，直接从 stdin 接收参数
process.stdin.on('data', (data) => {
    try {
        const args = JSON.parse(data.toString());

        if (args.length !== 8) {
            console.error("错误：参数数量不正确，请确保传入 8 个参数。");
            return;
        }
        ...
        const click = args[0];
        const xy = args[1];
        const pic_url = args[2];
        const nc = args[3];
        const ns = args[4];
        const gt = args[5];
        const challenge = args[6];
        const passtime = args[7];

        // 调用 getW 函数
        const result = getW(click, xy, pic_url, nc, ns, gt, challenge, passtime);

        // 输出结果
        console.log(result);
    } catch (err) {
        console.error("输入解析错误：", err.message);
    }
});

// // 提示程序结束
// process.on('exit', () => {
//     console.log("程序已退出，感谢使用！");
// });
