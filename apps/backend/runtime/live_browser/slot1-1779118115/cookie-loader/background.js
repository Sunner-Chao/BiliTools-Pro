...
async function run() {
  ...
  const first = tabs.find((tab) => tab.url === "about:blank") || tabs[0];
  if (first && first.id) {
    await chrome.tabs.update(first.id, { url: LIVE_URL, active: true });
    setTimeout(() => chrome.tabs.reload(first.id), 1500);
  } else {
