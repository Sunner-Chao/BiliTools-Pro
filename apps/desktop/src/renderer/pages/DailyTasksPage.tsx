...
import { Alert, Button, Card, Col, DatePicker, Divider, Form, Input, InputNumber, List, message, Modal, Row, Select, Space, Table, Tag, Typography } from 'antd';
import { CheckCircleOutlined, DeleteOutlined, EditOutlined, GiftOutlined, LoginOutlined, MessageOutlined, PlayCircleOutlined, QrcodeOutlined, ReloadOutlined, SaveOutlined, VideoCameraOutlined, WalletOutlined } from '@ant-design/icons';
import dayjs from 'dayjs';
import { useAppSelector } from '../store/hooks';

const DAILY_WORKFLOW_TIME_FORMAT = 'YYYY-MM-DDTHH:mm:ss';
...
const serializeWorkflowTime = (value: dayjs.Dayjs | undefined) => value?.format(DAILY_WORKFLOW_TIME_FORMAT);
const workflowCardStyle = { marginTop: 16 };
...
const DailyTasksPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    workflowForm.setFieldsValue({
      name: '每日进房打卡',
      roomId: user?.roomId ? String(user.roomId) : '',
      slot: 0,
      durationMinutes: 16,
      danmakuCount: 1,
      giftCount: 1,
      ...
      repeat: 'daily',
      enabled: true,
      actions: ['validate', 'wallet', 'enterLiveRoom'],
  ...
  const saveWorkflow = async () => {
    ...
    try {
      ...
      const result = await window.api.daily.saveWorkflow({
        ...values,
        targetTime: serializeWorkflowTime(values.targetTime),
      });
      if (result?.workflow?.id) workflowForm.setFieldValue('id', result.workflow.id);
      message.success('自动化工作流已保存');
      await loadStatus();
    } catch (error: any) {
      message.error(error?.error || error?.message || '工作流保存失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  const newWorkflow = () => {
    workflowForm.setFieldsValue({
      id: undefined,
      name: '每日进房打卡',
      roomId: user?.roomId ? String(user.roomId) : form.getFieldValue('roomId') || '',
      slot: 0,
      durationMinutes: 16,
      danmakuCount: 1,
      giftCount: 1,
    ...
    message.info('已切换为新建工作流');
  };

  const editWorkflow = (workflow: any) => {
    workflowForm.setFieldsValue({
      ...workflow,
      targetTime: workflow.targetTime ? dayjs(workflow.targetTime) : undefined,
    });
    message.info('已载入工作流配置');
  };

  const runWorkflowNow = async (workflowId: string) => {
    setLoading(true);
    try {
      await window.api.daily.runWorkflow(workflowId);
      message.success('工作流已执行');
      await loadStatus();
    } catch (error: any) {
      message.error(error?.error || error?.message || '工作流执行失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteWorkflow = (workflowId: string) => {
    Modal.confirm({
      ...
      onOk: async () => {
        setLoading(true);
        try {
          await window.api.daily.deleteWorkflow(workflowId);
  ...
  if (pageLoading) {
    return (
      <div className="bt-page-skeleton" role="status" aria-label="加载每日任务">
        ...
        <div className="bt-daily-skeleton-main bt-equal-row">
          <div className="bt-skeleton bt-stagger-1 bt-daily-skeleton-config" />
          <div className="bt-daily-skeleton-slots">
            {[0, 1, 2, 3].map(i => (
              <div key={i} className={`bt-skeleton bt-stagger-${i + 2} bt-daily-skeleton-slot`} />
            ))}
          </div>
        </div>
        <div className="bt-skeleton bt-stagger-6 bt-daily-skeleton-workflow" />
        <div className="bt-skeleton bt-stagger-7 bt-daily-skeleton-list" />
        <div className="bt-skeleton bt-stagger-8 bt-daily-skeleton-log" />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <Row gutter={[16, 16]} className="bt-equal-row">
        <Col xs={24} md={8}>
          <Card title="每日任务系统" className="bt-fill-card">
            <Form form={form} layout="vertical" className="bt-card-form">
              ...
              <Form.Item name="entryMode" label="进入方式">
                <Select
                  options={[
                    { value: 'browser', label: '可见浏览器进房（推荐，会显示观众）' },
                    { value: 'api', label: 'API 观看心跳（后台保活，不保证前台可见）' },
                    { value: 'headless', label: '无头浏览器兜底（不显示窗口）' },
                  ]}
                />
              </Form.Item>
        ...
        <Col xs={24} md={16}>
          <Card title="观众列表" className="bt-fill-card">
          <div className="bt-scroll-panel">
          <Row gutter={[12, 12]}>
      ...
      <Card title="自动化工作流" className="bt-workflow-card" style={workflowCardStyle}>
          ...
          <Row gutter={[16, 8]} align="bottom">
            <Col xs={24} sm={12} xl={5}>
              <Form.Item name="name" label="工作流名称" style={workflowFormItemStyle} rules={[{ required: true, message: '请输入工作流名称' }]}>
                <Input placeholder="例如：每日进房打卡" />
              </Form.Item>
            </Col>
            <Col xs={12} sm={6} xl={3}>
              <Form.Item name="slot" label="观众" style={workflowFormItemStyle} rules={[{ required: true, message: '请选择观众' }]}>
                <Select options={(status.slots || []).map((slot: any) => ({ value: slot.slot, label: `观众 ${slot.slot}${slot.name ? ` · ${slot.name}` : ''}` }))} />
              </Form.Item>
            </Col>
            <Col xs={12} sm={6} xl={3}>
              <Form.Item name="repeat" label="触发方式" style={workflowFormItemStyle}>
                <Select options={[{ value: 'daily', label: '每天' }, { value: 'once', label: '仅一次' }]} />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12} xl={5}>
            ...
            <Col xs={24} sm={12} xl={4}>
              <Form.Item name="roomId" label="直播间号" style={workflowFormItemStyle} rules={[{ required: true, message: '请输入直播间号' }]}>
                <Input placeholder="直播间号" />
              </Form.Item>
            </Col>
            <Col xs={12} sm={6} xl={2}>
            ...
            <Col xs={12} sm={6} xl={2}>
              <Form.Item name="durationMinutes" label="保活" style={workflowFormItemStyle}>
                <InputNumber min={1} max={240} style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col xs={12} sm={6} xl={2}>
              <Form.Item name="danmakuCount" label="弹幕" style={workflowFormItemStyle}>
                <InputNumber min={1} max={20} style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col xs={12} sm={6} xl={2}>
              <Form.Item name="giftCount" label="牛蛙" style={workflowFormItemStyle}>
                <InputNumber min={1} max={20} style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12} xl={4}>
              <Form.Item name="entryMode" label="进入方式" style={workflowFormItemStyle}>
                <Select
                  options={[
                    { value: 'browser', label: '可见浏览器' },
                    { value: 'api', label: 'API心跳' },
            ...
            <Col xs={24} xl={7}>
              <Form.Item name="actions" label="动作序列" style={workflowFormItemStyle} rules={[{ required: true, message: '请选择至少一个动作' }]}>
                <Select
                  mode="multiple"
                  options={[
                    { value: 'validate', label: workflowActionLabels.validate },
                    { value: 'wallet', label: workflowActionLabels.wallet },
                    { value: 'enterLiveRoom', label: workflowActionLabels.enterLiveRoom },
                    { value: 'sendDanmaku', label: workflowActionLabels.sendDanmaku },
                    { value: 'sendGift', label: workflowActionLabels.sendGift },
                  ]}
                />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12} xl={5}>
              <Form.Item name="message" label="弹幕内容" style={workflowFormItemStyle}>
                <Input placeholder="留空则随机" />
              </Form.Item>
            </Col>
            <Col xs={24} xl={4}>
              <Form.Item label="操作" style={workflowFormItemStyle}>
                <Space wrap>
                  <Button type="primary" icon={<SaveOutlined />} loading={loading} onClick={saveWorkflow}>保存</Button>
      ...
      <Card title="已保存自动化工作流" className="bt-workflow-card" style={{ marginTop: 16 }} extra={<Button icon={<ReloadOutlined />} onClick={loadStatus}>刷新</Button>}>
        <List
              ...
              <List.Item.Meta
                title={(
                  <Space wrap>
                    ...
                    <Tag color={workflow.enabled ? 'green' : 'default'}>{workflow.enabled ? '启用' : '停用'}</Tag>
                    <Tag color={workflow.repeat === 'daily' ? 'blue' : 'gold'}>{workflow.repeat === 'daily' ? '每天' : '仅一次'}</Tag>
                    <Tag color={workflow.lastStatus === 'error' ? 'red' : workflow.lastStatus === 'success' ? 'green' : 'default'}>{workflow.lastStatus || 'idle'}</Tag>
                  </Space>
                )}
                description={(
                  <Space direction="vertical" size={2}>
                    <Typography.Text style={{ color: 'var(--bt-text-secondary)', fontSize: 12 }}>
                      观众 {workflow.slot} · 房间 {workflow.roomId} · 下次触发 {workflow.nextRunAt || '-'}
                    </Typography.Text>
                    <Typography.Text style={{ color: 'var(--bt-text-secondary)', fontSize: 12 }}>
                      {(workflow.actions || []).map((action: string) => workflowActionLabels[action] || action).join(' → ')}
                    </Typography.Text>
      ...
      <Card title="每日任务日志" className="bt-workflow-card" style={{ marginTop: 16 }} extra={<Button icon={<ReloadOutlined />} onClick={loadStatus}>刷新</Button>}>
        <Table rowKey={(_, index) => `daily-log-${index}`} size="small" columns={logColumns} dataSource={status.logs || []} pagination={{ pageSize: 10 }} scroll={{ y: 280 }} locale={{ emptyText: '暂无日志' }} />
      </Card>
